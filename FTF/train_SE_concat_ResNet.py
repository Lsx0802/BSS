...
def save_TSNE(label_TSNE,val_TSNE_1,val_TSNE_2,epoch,name):
    ...
    plot_with_labels(low_dim_embs_1, low_dim_embs_2, labels, epoch, name)
def plot_with_labels(lowDWeights_1, lowDWeights_2, labels, epoch, name):
    ...
    for x, y, s in zip(X_2, Y_2, labels):
        c = cm.rainbow(int(255 * (s + 2) / 4))

        plt.text(x, y, str(s), color=c, fontdict={'weight': 'bold', 'size': 9})  # 在指定位置放置文本
        plt.xlim(min(X_1.min(), X_2.min()), max(X_1.max(), X_2.max()))
        plt.ylim(min(Y_1.min(), Y_2.min()), max(Y_1.max(), Y_2.max()))
        plt.axis('off')
        plt.plot()
...
def main():
    ...
    for epoch in range(EPOCH):
        ...
        with torch.no_grad():
            ...
            save_TSNE(label_TSNE, val_TSNE_1, val_TSNE_2, epoch, name)
