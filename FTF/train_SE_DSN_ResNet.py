...
from DSN_SE_ResNet import resnet34
import torch
...
def save_TSNE(label_TSNE,val_TSNE_1,val_TSNE_2,val_TSNE_3,epoch,name):
    label_TSNE = np.array(label_TSNE)
    val_TSNE_1 = np.array(val_TSNE_1)
    val_TSNE_2 = np.array(val_TSNE_2)
    val_TSNE_3 = np.array(val_TSNE_3)
    tsne = TSNE(perplexity=30, n_components=2, init='pca', n_iter=5000)
    # 最多只画500个点
    plot_only = 75
    # fit_transform函数把last_layer的Tensor降低至2个特征量,即3个维度(2个维度的坐标系)
    low_dim_embs_1 = tsne.fit_transform(val_TSNE_1[:plot_only, :])
    low_dim_embs_2 = tsne.fit_transform(val_TSNE_2[:plot_only, :])
    low_dim_embs_3 = tsne.fit_transform(val_TSNE_3[:plot_only, :])
    labels = label_TSNE[:plot_only]
    plot_with_labels(low_dim_embs_1, low_dim_embs_2,low_dim_embs_3, labels, epoch, name)
    
def plot_with_labels(lowDWeights_1, lowDWeights_2, lowDWeights_3,labels, epoch, name):
    plt.cla()  # clear当前活动的坐标轴
    ####face
    X_1, Y_1 = lowDWeights_1[:, 0], lowDWeights_1[:, 1]  # 把Tensor的第1列和第2列,也就是TSNE之后的前两个特征提取出来,作为X,Y
    for x, y, s in zip(X_1, Y_1, labels):
        c = cm.rainbow(int(255 * s / 6))
    ...
    plt.plot()
    #############tongue
    X_2, Y_2 = lowDWeights_2[:, 0], lowDWeights_2[:, 1]  # 把Tensor的第1列和第2列,也就是TSNE之后的前两个特征提取出来,作为X,Y
    for x, y, s in zip(X_2, Y_2, labels):
        c = cm.rainbow(int(255 * (s + 2) / 6))

        plt.subplot(1, 3, 2)
        plt.text(x, y, str(s), color=c, fontdict={'weight': 'bold', 'size': 9})  # 在指定位置放置文本
    plt.xlim(X_2.min(), X_2.max())
    plt.ylim(Y_2.min(), Y_2.max())
    plt.axis('off')
    plt.plot()
    ############concat
    X_3, Y_3 = lowDWeights_3[:, 0], lowDWeights_3[:, 1]  # 把Tensor的第1列和第2列,也就是TSNE之后的前两个特征提取出来,作为X,Y
    for x, y, s in zip(X_3, Y_3, labels):
        c = cm.rainbow(int(255 * (s + 4) / 6))

        plt.subplot(1, 3,3)
        plt.text(x, y, str(s), color=c, fontdict={'weight': 'bold', 'size': 9})  # 在指定位置放置文本
    plt.xlim(X_3.min(), X_3.max())
    plt.ylim(Y_3.min(), Y_3.max())
    plt.axis('off')
    plt.plot()


    path = 'save_img/TSNE_' + name
...
def main():
    ...
    for epoch in range(EPOCH):
        ...
        model.eval()
        with torch.no_grad():
            for i, (val_data_batch_face, val_data_batch_tongue, val_label_batch) in tqdm(enumerate(data_loader_val),
                ...
                label_TSNE.extend(val_label_batch.detach().cpu().numpy())
                val_TSNE_1.extend(val_outputs2.detach().cpu().numpy())
                val_TSNE_2.extend(val_outputs3.detach().cpu().numpy())
                val_TSNE_3.extend(val_outputs3.detach().cpu().numpy())
            ...
            save_TSNE(label_TSNE, val_TSNE_1, val_TSNE_2, val_TSNE_3,epoch, name)
            ...
            print(
                "[valadation] Epoch:{} accuracy:{:.2f} precision:{:.2f} recall:{:.2f} f1:{:.2f} AUC:{:.2f} loss:{:.4f} ".format(
