...
def plot_with_labels(lowDWeights, labels,epoch,name):
...
def main():
    ...
    train_txt_path = 'txt/train3.txt'
    val_txt_path = 'txt/val3.txt'
    name = 'SE_face_tongue_448_fold32'
    ...
    for epoch in range(EPOCH):
        ...
        with torch.no_grad():
            for i, (val_data_batch, val_label_batch) in tqdm(enumerate(data_loader_val), total=len(data_loader_val)):
                ...
                val_TSNE.extend(val_outputs.detach().cpu().numpy())
            ...
            if accuracy >= best_accuracy:
                ...
                val_TSNE = np.array(val_TSNE)
                tsne = TSNE(perplexity=30, n_components=2, init='pca', n_iter=5000)
                # 最多只画500个点
                plot_only = 75
                # fit_transform函数把last_layer的Tensor降低至2个特征量,即3个维度(2个维度的坐标系)
                low_dim_embs = tsne.fit_transform(val_TSNE[:plot_only, :])
                labels = label_TSNE[:plot_only]
                plot_with_labels(low_dim_embs, labels, epoch, name)
